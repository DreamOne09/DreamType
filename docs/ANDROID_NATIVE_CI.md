...
驗證程式僅放在可拋棄測試 APK。0.9.11 產品新增 Application 啟動清理：在任何錄音元件啟動前，只嘗試刪除私有快取中 voice-*.m4a 檔案；不刪除其他快取或 noBackupFilesDir 的加密保留錄音。測試另預置未完成明文暫存及其他快取，要求新程序清除前者、保留後者。舊版在錄音途中程序被關閉時可能遺留明文快取；新版離線隱私頁補上這項資料生命週期。當前原生執行結果待確認，尚未發布 0.9.11。它驗證完整寫入後的跨程序保存，不模擬寫到一半遭終止、裝置重開機、伺服器未完成工作恢復或長時間離線，也不使用真人錄音。
