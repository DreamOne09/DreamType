...
首次可下載固定版本的 Google bundletool 1.18.3、Google Maven AAPT2 9.4.1-15978811，並檢查程式內固定的 SHA-256；已下載後可省略參數。沿用專案 JDK、Android 36 平台與 D8。每次使用全新的暫存編譯目錄，不混入舊 class／dex。

產物：

- `outputs/android/DreamType-0.9.0-experimental.aab`：商店建置候選，不能直接安裝到手機。
- `outputs/android/DreamType-0.9.0-bundle-test.apk`：由 AAB 產生的本機測試安裝檔，不取代 GitHub 已發布 APK。
...
套件仍為 `tw.localvoice.keyboard`，versionCode 10／versionName 0.9.0。本次衍生 APK 的簽章 SHA-256 為 `8af01d69be8604c2f4a288d55651bb72bed42a391c546f04db4a2a0deb14fc98`，與既有 APK 相同。Google Play App Signing 的註冊／金鑰策略尚未設定；必須在第一次上架前確認，不能僅凭本機同簽章推斷商店安裝一定能覆蓋側載版本。
...
- 0.9.0 候選產物 compile／target SDK 已升至 36，最低支援仍為 API 26。2026-09-22 查核 [Google Play 官方要求](https://support.google.com/googleplay/android-developer/answer/11926878?hl=en)：自 2026-08-31 起一般手機新 App／更新需 target API 36。目前已符合目標 API 數值門檻，但 Android 16 行為仍待裝置驗證。AAB 格式通過不等於商店政策通過。
