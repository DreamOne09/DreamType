...
維護順序為先建立／複製每日備份，再匯出／複製刪除紀錄。新備份內含快照完成時間，還原會拒絕匯出時間更早的紀錄。沒有該時間欄位的歷史備份仍依同主機核對及人工確認最新來源處理。這項檢查依賴主機時鐘，只能排除已知較舊紀錄，不能證明它包含快照之後的全部刪除。

複製至同步資料夾使用同資料夾暫存檔，完整寫入後才替換目的檔；複製失敗保留上一份完整檔。遠端雲端的上傳順序、原子性與完成狀態仍由同步工具決定，尚未驗證。若先收到新備份但刪除紀錄尚未更新，還原應被拒絕，待取得新紀錄再重試。
