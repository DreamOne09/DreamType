...
更新日期：2026-09-20。狀態：產品方向與架構規劃，尚未實作／部署。
...
- 現階段先更新 GitHub 文件，不新增真實收費、不申請帳號、不部署雲端服務。

「兩到三個工具」目前先定義為可替換的服務或功能模組；尚未指定是哪幾個產品。如果後續包含外部 App 操作或其他工作流程，另行定義其介面及權限，不預設加入。

## 現況與目標分開

| 項目 | 目前 0.3.1 試用版 | 商店版目標 |
|---|---|---|
| 語音處理 | 使用者的 Windows NVIDIA 電腦 | 我們提供的雲端服務 |
| 使用者識別 | 私人連線金鑰，並非帳號系統 | 經驗證的使用者帳號 |
| 提示詞／詞庫 | 每支手機私有設定 | 帳號隔離，可在換機後恢復 |
...
先評估 Google 登入，登入供應商尚未選定。無論使用 Firebase Auth、其他託管服務或自建方案，後端都必須驗證 token 的簽發者、對象、有效期，並映射到自己的 user_id。App 上顯示登入畫面不等於帳號系統完成。
