...
| 錄音 | 主機記憶體處理；Android 帳號模式保留一份加密重試錄音，最長一小時，詳見 0.6.0 文件 |
...
| 結果可靠交付 | 加密結果與扣額度同一 SQLite 交易；重啟仍可取回；0.7.0 收件確認、未確認到期釋放額度；同 ID 不重扣 | 處理中的音訊佇列仍只在 RAM，重啟標失敗退款，由手機手動重試；不是持久音訊佇列 |
...
- 25 項 Python 測試通過：跨重啟結果、交易回滾、收件確認隔離／冪等／退款、密碼恢復、備份竄改拒絕、恢復不覆蓋、維護重啟節流等。
