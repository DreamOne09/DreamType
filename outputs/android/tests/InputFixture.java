...
public final class InputFixture extends Activity {
 @Override public void onCreate(Bundle saved){super.onCreate(saved);
  ...
  EditText normal=new EditText(this);normal.setHint("一般文字");normal.setContentDescription("一般文字");normal.setInputType(InputType.TYPE_CLASS_TEXT|InputType.TYPE_TEXT_FLAG_MULTI_LINE);page.addView(normal,new LinearLayout.LayoutParams(-1,80));
  EditText password=new EditText(this);password.setHint("密碼欄位");password.setContentDescription("密碼欄位");password.setInputType(InputType.TYPE_CLASS_TEXT|InputType.TYPE_TEXT_VARIATION_PASSWORD);page.addView(password,new LinearLayout.LayoutParams(-1,80));
