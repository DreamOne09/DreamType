...
Post-0.8 backend schema is now 4: account deletion writes a uid-only tombstone in the same transaction. Encrypted .dtledger export is separate from snapshots; restore requires the latest same-host ledger and removes deleted users before installation. See docs/DELETION_RECOVERY.md for export lag, pre-upgrade deletion gaps, and off-machine limitations. Never commit ledgers; never claim a local copy proves cloud upload or that an arbitrary ledger is latest.
